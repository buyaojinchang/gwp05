#!/bin/bash
# Launch evaluation clients with lerobot data collection enabled.
# Usage: bash parallel_client_collect.sh [TASK_SET] [COLLECT_DIR]
#
# This is the same as parallel_client.sh but additionally passes
# --collect_data and --collect_dir to inference_client_collect.py.
# Each env's data is saved to: <COLLECT_DIR>/<env_name>/lerobot/
#
...
TASK_SET="${1:-composite_unseen}"
COLLECT_DIR="${2:-/shared_disk/users/hengtao.li/robocasa_datasets/collected}"
SPLIT="pretrain"
NUM_TRIALS=50
REPLAN_STEPS=20
# Client-side seed for np.random + robocasa env init.
# Decoupled from server SEED on purpose: matches inference_client_collect.py's argparse default.
CLIENT_SEED="${3:-7}"
...
# Server SEED only controls sample noise; client SEED controls np.random + env init.
unset SEED
...
# Append model identity to collect dir so different checkpoints don't collide
COLLECT_DIR="${COLLECT_DIR}/${EXP_NAME}/${CKPT_NAME}/${MODEL_NAME}"
...
echo "============================================================"
echo "  Launching $NUM_WORKERS clients (with DATA COLLECTION)"
...
echo "  Seed:  $CLIENT_SEED (np.random + robocasa env seed; shared across all workers, tasks are sharded by worker_id)"
...
for i in $(seq 0 $((NUM_WORKERS - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=$GPU python -u experiment/robocasa/inference_client_collect.py \
        --port $PORT \
        --task_set $TASK_SET \
        --split $SPLIT \
...
echo "  Collected data:  $COLLECT_DIR/"
echo "============================================================"
