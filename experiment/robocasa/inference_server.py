"""GigaWorld-Policy inference server for RoboCasa evaluation.
...
def build_model(
    ...
    if not any(k.startswith("transformer.mot.") or k.startswith("mot.") or ".mot." in k for k in keys):
        raise ValueError(
            "This server only supports gwp-mot checkpoints. "
...
class GWPPolicy:
    ...
    def _infer_unlocked(self, obs: dict) -> dict:
        ...
        logger.info(f"Action[0] (denormed, pre-postproc): {actions_np[0]}")

        # 8. Post-processing for robocasa
        ...
        logger.info(f"Inference took {elapsed*1000:.1f}ms, action shape: {actions_np.shape}, "
...
class InferenceServer:
    ...
    async def _handler(self, websocket):
        ...
        # Send metadata
        await websocket.send(packer.pack({"model": "gwp-mot"}))
...
def parse_args():
    parser = argparse.ArgumentParser(description="gwp-mot MoT RoboCasa inference server")
    ...
    parser.add_argument("--zero_action_dims", type=int, nargs="*", default=None,
                        help="Action dims to force to 0 (e.g. 3 for robocasa ee_rot_rx)")
    parser.add_argument("--ctrl_mode_dim", type=int, default=None,
                        help="Action dim index for control_mode (e.g. 11 for robocasa)")
    ...
    parser.add_argument("--tshape", action="store_true", default=True,
                        help="Compatibility flag; MoT RoboCasa server always uses T-shape layout")
...
def main():
    ...
    print("=" * 60)
    print("GigaWorld-Policy RoboCasa Inference Server")
    print("=" * 60)
