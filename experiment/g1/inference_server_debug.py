"""[DEBUG BUILD] GWP-MoT inference server for the G1 locomanip pick_place (g1_sonic) task.

This is a standalone *debug* copy of ``inference_server.py`` (kept identical in
behavior, plus extra ``--debug-io`` I/O logging). The original
``inference_server.py`` is left unchanged; run this module via
``inference_server_debug.sh`` to diagnose issues such as "only the gripper moves"
(it prints whether the sonic-latent state token is zero and how much the
motion_token actually varies over the horizon).
...
class ServerConfig:
    ...
    debug_io: bool = False
    """Print a structured summary of every received observation and returned
    action (keys, shapes, dtypes, value ranges) so wire I/O can be verified."""
...
def _describe_value(value: Any) -> str:
    """One-line summary of an observation value (shape/dtype/range/preview)."""
    if isinstance(value, np.ndarray):
        extra = ""
        if value.size and np.issubdtype(value.dtype, np.number):
            extra = f" range=[{value.min():.3g},{value.max():.3g}]"
        return f"ndarray{list(value.shape)} {value.dtype}{extra}"
    if isinstance(value, torch.Tensor):
        return f"tensor{list(value.shape)} {value.dtype}"
    if isinstance(value, dict):
        return "dict{" + ", ".join(map(str, value.keys())) + "}"
    if isinstance(value, (list, tuple)):
        return f"{type(value).__name__}(len={len(value)})"
    if isinstance(value, (bytes, bytearray)):
        text = bytes(value).decode("utf-8", errors="ignore")
        return f"bytes({text[:48]!r})"
    if isinstance(value, str):
        return f"str({value[:48]!r})"
    return f"{type(value).__name__}({value!r})"


def _temporal_variation(arr: np.ndarray) -> tuple[float, float]:
    """Return (mean per-dim std over time, mean |frame-to-frame delta|).

    For a [T, D] action chunk, both numbers go to ~0 when the predicted track
    is static over the horizon (i.e. the corresponding body part does not move).
    """
    if arr.ndim != 2 or arr.shape[0] < 2:
        return 0.0, 0.0
    std_over_time = float(arr.std(axis=0).mean())
    step_delta = float(np.abs(np.diff(arr, axis=0)).mean())
    return std_over_time, step_delta


def _describe_observation(observation: Any, indent: str = "    ") -> str:
    """Multi-line structural dump of a (possibly nested) observation dict."""
    lines: list[str] = []
        ...
        for key, val in observation.items():
            if isinstance(val, dict):
                ...
                for sub_key, sub_val in val.items():
                    lines.append(f"{indent}  {sub_key}: {_describe_value(sub_val)}")
            else:
                lines.append(f"{indent}{key}: {_describe_value(val)}")
    else:
        lines.append(f"{indent}{_describe_value(observation)}")
...
class G1SonicPolicy:
    ...
    def get_action(
        ...
        if cfg.debug_io:
            print(f"[G1Server][IO] ── get_action req #{self._request_count} ──────────")
            print("[G1Server][IO] received observation:")
            print(_describe_observation(observation))
        ...
        if latent_row is None:
            ...
            latent_src = "zeros"  # smoking gun for static body (token1 not fed at deploy)
        ...
        info = {
            ...
            "latent_state_is_zero": bool(latent_src == "zeros" or not np.any(latent_row)),
            "motion_time_std": float(_temporal_variation(motion_token)[0]),
            "motion_step_delta": float(_temporal_variation(motion_token)[1]),
        }

        if cfg.debug_io:
            m_std, m_step = _temporal_variation(motion_token)
            h_std, h_step = _temporal_variation(hand_binary)
            latent_zero = latent_src == "zeros" or not np.any(latent_row)
            print(
                f"[G1Server][IO] inputs: ego_frame={list(ego_frame.shape)} {ego_frame.dtype} "
                f"range=[{int(ego_frame.min())},{int(ego_frame.max())}] | "
                f"joint(src={joint_src})[:5]={np.round(joint_row[:5], 3).tolist()} | "
                f"latent(src={latent_src}) absmean={np.abs(latent_row).mean():.4f}"
                f"{'  <-- ALL ZERO (train/deploy mismatch: body likely static)' if latent_zero else ''} | "
                f"text(src={text_src}) prompt={prompt!r}"
            )
            print(
                f"[G1Server][IO] returning action: "
                f"motion_token{list(action['motion_token'].shape)} "
                f"hand_binary{list(action['hand_binary'].shape)} (horizon={motion_token.shape[0]})"
            )
            print(
                f"[G1Server][IO]   motion_token absmean={np.abs(motion_token).mean():.4f} "
                f"max={np.abs(motion_token).max():.4f} "
                f"time_std={m_std:.4f} step|Δ|={m_step:.4f}"
                f"{'  <-- ~STATIC over horizon (body not moving)' if m_step < 1e-3 else ''}"
            )
            print(
                f"[G1Server][IO]   hand_binary mean=[{hand_binary[:, 0].mean():.3f},{hand_binary[:, 1].mean():.3f}] "
                f"range=[{hand_binary.min():.3f},{hand_binary.max():.3f}] "
                f"time_std={h_std:.4f} step|Δ|={h_step:.4f} | {elapsed * 1000:.0f}ms"
            )
    ...
    def run(self) -> None:
        ...
        while self.running:
            try:
                ...
                if getattr(self.policy.cfg, "debug_io", False):
                    data_keys = list(data.keys()) if isinstance(data, dict) else data
                    print(f"[G1Server][IO] <- endpoint={endpoint!r} data_keys={data_keys}")
