#!/usr/bin/env bash
# [DEBUG BUILD] Launch the G1 locomanip pick_place (g1_sonic) GWP-MoT inference
# server with verbose I/O logging. This is a standalone debug copy; the original
# inference_server.sh / inference_server.py are kept unchanged.
#   - logs stdout/stderr to a timestamped file (LOG_DIR / LOG_FILE)
#   - DEBUG_IO defaults to true: dumps each received observation + returned action
#   - runs the debug module: experiment.g1.inference_server_debug
#
...
export DEBUG_IO="${DEBUG_IO:-true}"              # dump received obs + returned action (shapes/ranges)

# 日志：终端输出同时写入文件。LOG_DIR 默认在脚本同目录的 logs/ 下，
# LOG_FILE 默认带时间戳；设 LOG_FILE 为空字符串可关闭落盘。
export LOG_DIR="${LOG_DIR:-}"
export LOG_FILE="${LOG_FILE-__AUTO__}"
...
Defaults (edit in script or export before run):
  ...
  DEBUG_PRINT_STATS, DEBUG_IO, LOG_DIR, LOG_FILE

Logging / debug:
  DEBUG_IO=true        dump each received observation + returned action (shapes/ranges)
  DEBUG_PRINT_STATS=true  print compact per-request latent/action stats
  LOG_DIR=/path        directory for the timestamped log file (default: <script_dir>/logs)
  LOG_FILE=/path/x.log explicit log file path; LOG_FILE="" disables file logging
...
if [ "${DEBUG_IO}" = "true" ] || [ "${DEBUG_IO}" = "1" ]; then
    PY_ARGS+=(--debug-io)
fi
if [ -n "${SEED}" ]; then
    PY_ARGS+=(--seed "${SEED}")
...
# ── 日志落盘：将后续 stdout/stderr 同时写入终端与日志文件 ────────────────────
LOG_DIR="${LOG_DIR:-${SCRIPT_DIR}/logs}"
if [ "${LOG_FILE}" = "__AUTO__" ]; then
    LOG_FILE="${LOG_DIR}/inference_server_$(date +%Y%m%d_%H%M%S).log"
fi
if [ -n "${LOG_FILE}" ]; then
    mkdir -p "$(dirname "${LOG_FILE}")"
    # tee 到日志文件；同时保留终端输出。pipefail 下用 process substitution 避免吞掉退出码。
    exec > >(tee -a "${LOG_FILE}") 2>&1
fi

echo "============================================================"
echo "  G1 Sonic GWP-MoT Inference Server [DEBUG BUILD] (GR00T PolicyClient compatible)"
...
echo "  Seed       : ${SEED:-<none>}"
echo "  Debug IO   : ${DEBUG_IO}  (debug_print_stats=${DEBUG_PRINT_STATS})"
echo "  Log file   : ${LOG_FILE:-<disabled>}"
echo "  Extra args : ${EXTRA_ARGS[*]:-<none>}"
echo "============================================================"

cd "${GWP_ROOT}"
exec "${SERVER_PYTHON}" -u -m experiment.g1.inference_server_debug "${PY_ARGS[@]}"
